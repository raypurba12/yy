<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 dark:text-gray-200 leading-tight">
            {{ __('Edit Pembelian dari Supplier') }}
        </h2>
    </x-slot>
        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-xl overflow-hidden border border-gray-200 dark:border-gray-700">
                <div class="p-6 border-b border-gray-200 dark:border-gray-700">
                    <h3 class="text-xl font-bold text-gray-800 dark:text-white">Edit Pembelian dari Supplier</h3>
                    <p class="mt-1 text-gray-600 dark:text-gray-400">Perbarui data pembelian dari supplier</p>
                </div>

                <form id="restockForm" method="POST" action="{{ route('restocks.update', $restock) }}">
                    @csrf
                    @method('PUT')
                    <div class="p-6 border-b border-gray-200 dark:border-gray-700">
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div>
                                ...
                                <select name="supplier_id" id="supplier_id" required class="w-full rounded-xl border border-gray-300 bg-white py-2 px-3 text-gray-900 focus:border-cyan-500 focus:ring-cyan-500 dark:border-gray-600 dark:bg-gray-700 dark:text-white">
                                    ...
                                    @foreach($suppliers as $supplier)
                                        <option value="{{ $supplier->id }}" {{ $restock->supplier_id == $supplier->id ? 'selected' : '' }}>{{ $supplier->name }}</option>
                                    @endforeach
                                </select>
                            </div>
                            ...
                            <div>
                                ...
                                <input type="date" name="purchase_date" id="purchase_date" value="{{ old('purchase_date', $restock->purchase_date) }}" required class="w-full rounded-xl border border-gray-300 bg-white py-2 px-3 text-gray-900 focus:border-cyan-500 focus:ring-cyan-500 dark:border-gray-600 dark:bg-gray-700 dark:text-white">
                            </div>
                        </div>

                        <div class="mt-6">
                            <label for="notes" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Catatan Tambahan</label>
                            <textarea name="notes" id="notes" rows="3" class="w-full rounded-xl border border-gray-300 bg-white py-2 px-3 text-gray-900 focus:border-cyan-500 focus:ring-cyan-500 dark:border-gray-600 dark:bg-gray-700 dark:text-white">{{ old('notes', $restock->notes) }}</textarea>
                        </div>
                    </div>
                    ...
                    <div class="p-6 border-b border-gray-200 dark:border-gray-700">
                        ...
                        <div id="itemsContainer">
                            ...
                            @foreach($restock->restockItems as $index => $item)
                            <div class="item-row grid grid-cols-1 md:grid-cols-12 gap-4 mb-4 p-4 bg-gray-50 dark:bg-gray-700 rounded-xl">
                                <div class="md:col-span-4">
                                    ...
                                    <select name="restock_items[{{ $index }}][product_id]" class="item-product w-full rounded-xl border border-gray-300 bg-white py-2 px-3 text-gray-900 focus:border-cyan-500 focus:ring-cyan-500 dark:border-gray-600 dark:bg-gray-700 dark:text-white" required>
                                ...
                                <div class="md:col-span-2">
                                    ...
                                    <input type="number" name="restock_items[{{ $index }}][price]" class="item-price w-full rounded-xl border border-gray-300 bg-white py-2 px-3 text-gray-900 focus:border-cyan-500 focus:ring-cyan-500 dark:border-gray-600 dark:bg-gray-700 dark:text-white" step="0.01" min="0" value="{{ $item->price }}" required>
                                </div>
                                
                                <div class="md:col-span-2">
                                    <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Jumlah (Kg)</label>
                                    <input type="number" name="restock_items[{{ $index }}][quantity]" class="item-quantity w-full rounded-xl border border-gray-300 bg-white py-2 px-3 text-gray-900 focus:border-cyan-500 focus:ring-cyan-500 dark:border-gray-600 dark:bg-gray-700 dark:text-white" step="0.01" min="0.01" value="{{ $item->quantity }}" required>
                                </div>
                                
                                <div class="md:col-span-2">
                                    <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Subtotal</label>
                                    <input type="text" class="item-subtotal w-full rounded-xl border border-gray-300 bg-white py-2 px-3 text-gray-900 focus:border-cyan-500 focus:ring-cyan-500 dark:border-gray-600 dark:bg-gray-700 dark:text-white" readonly value="{{ $item->subtotal }}">
                                </div>
                        ...
                        <div class="mt-4">
                            ...
                            <input type="text" id="totalAmount" class="w-full rounded-xl border border-gray-300 bg-white py-2 px-3 text-gray-900 focus:border-cyan-500 focus:ring-cyan-500 dark:border-gray-600 dark:bg-gray-700 dark:text-white" readonly value="{{ $restock->total_amount }}">
                        </div>
                    </div>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div>
                                ...
                                <select name="payment_status" id="payment_status" class="w-full rounded-xl border border-gray-300 bg-white py-2 px-3 text-gray-900 focus:border-cyan-500 focus:ring-cyan-500 dark:border-gray-600 dark:bg-gray-700 dark:text-white" required>
                                    <option value="belum_dibayar" {{ $restock->payment_status === 'belum_dibayar' ? 'selected' : '' }}>Belum Dibayar</option>
                                    <option value="dibayar" {{ $restock->payment_status === 'dibayar' ? 'selected' : '' }}>Dibayar</option>
                                    <option value="gagal" {{ $restock->payment_status === 'gagal' ? 'selected' : '' }}>Gagal</option>
                                </select>
                            </div>

                            <div id="paymentMethodSection" class="{{ $restock->payment_status === 'dibayar' ? '' : 'hidden' }}">
                                ...
                                <select name="payment_method" id="payment_method" class="w-full rounded-xl border border-gray-300 bg-white py-2 px-3 text-gray-900 focus:border-cyan-500 focus:ring-cyan-500 dark:border-gray-600 dark:bg-gray-700 dark:text-white">
                                    <option value="cash" {{ $restock->payment_method === 'cash' ? 'selected' : '' }}>Cash</option>
                                    <option value="transfer" {{ $restock->payment_method === 'transfer' ? 'selected' : '' }}>Transfer</option>
                                    <option value="qris" {{ $restock->payment_method === 'qris' ? 'selected' : '' }}>QRIS</option>
                                </select>
                            </div>
                        </div>

                        <!-- Cash Payment Details -->
                        <div id="cashPaymentDetails" class="{{ $restock->payment_method === 'cash' && $restock->payment_status === 'dibayar' ? '' : 'hidden' }} mt-6">
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                <div>
                                    <label for="received_amount" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Jumlah Uang</label>
                                    <input type="number" name="received_amount" id="received_amount" class="w-full rounded-xl border border-gray-300 bg-white py-2 px-3 text-gray-900 focus:border-cyan-500 focus:ring-cyan-500 dark:border-gray-600 dark:bg-gray-700 dark:text-white" step="0.01" min="0" value="{{ $restock->received_amount }}">
                                </div>
                                
                                <div>
                                    <label for="change_amount" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Kembalian</label>
                                    <input type="text" id="change_amount" class="w-full rounded-xl border border-gray-300 bg-white py-2 px-3 text-gray-900 focus:border-cyan-500 focus:ring-cyan-500 dark:border-gray-600 dark:bg-gray-700 dark:text-white" readonly value="{{ $restock->change_amount }}">
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="p-6 flex justify-end space-x-4">
                        <a href="{{ route('restocks.index') }}" class="px-6 py-3 bg-gray-100 hover:bg-gray-200 text-gray-700 font-medium rounded-xl transition duration-300 dark:bg-gray-700 dark:hover:bg-gray-600 dark:text-white">
                            Batal
                        </a>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            // Counter for new item rows - we need to make sure it starts after existing items
            let itemIndex = {{ count($restock->restockItems) }};
